  ...
  const rows = useMemo<InternalDeviceRow[]>(
    () =>
      sortInternalDevicesByCreationDate(devices).map((device, index) => ({
        ...
        status_label: device.device_status?.name ?? 'SIN ESTATUS',
        assignment_label: device.assigned ? 'ASIGNADO' : 'SIN ASIGNAR',
        review_label: device.reviewed ? 'REVISADO' : 'SIN REVISION',
