...
const withInvoiceContext = (
  ...
  if (context.requisitionCode)
    params.set("requisitionCode", context.requisitionCode);
...
export const getTabsFromPath = (
  ...
  const tabsMap: Record<string, Tab[]> = {
    ...
    "proyects/proyects": [
      {
        ...
        path: "/main-page/proyects/proyects/newproyect",
      },
    ...
    "proyects/inventory": [
      ...
      {
        label: "Refacciones",
        path: "/main-page/proyects/inventory/refactions",
      },
    ...
    "generalservices/vehicleregist": [
      {
        ...
        path: "/main-page/generalservices/vehicleregist/vehicleregistry",
      },
      {
        label: "Lista de Registros",
        path: "/main-page/generalservices/vehicleregist/vehicleregistrylist",
      },
    ],
    "humanresources/organizationchart": [
      {
        label: "Departamentos",
        path: "/main-page/humanresources/organizationchart/departments",
      },
      {
        label: "Directorio General",
        path: "/main-page/humanresources/organizationchart/generaldirectory",
      },
    ],
    organigrama: [
    ...
    "humanresources/release": [
      {
        label: "Comunicados",
        path: "/main-page/humanresources/release/pressreleases",
      },
      {
        label: "Información Importante",
        path: "/main-page/humanresources/release/importantinformation",
      },
    ],
    "administration/usersmanagment": [
      {
        label: "Crear Empleado",
        path: "/main-page/administration/usersmanagment/createemployee",
      },
      {
        label: "Lista de Empleados",
        path: "/main-page/administration/usersmanagment/employeesList",
      },
    ],
    authorizations: [
      {
  ...
  const normalizePersonLabel = (label?: string | null) => {
    ...
    const prefixed =
      ...
      lower.startsWith("requisiciones ") ||
      lower.startsWith("detalle ");
  ...
  if (search) {
    ...
    authorizationDetailLabel = normalizePersonLabel(
      sp.get("authorizationDetailLabel"),
    );
  ...
  const internalDevicesListPath =
    "/main-page/it/internaldevices/internaldeviceslist";
  const internalDevicesAssignPath =
  ...
  if (first === "it" && second === "internaldevices") {
      ...
      if (view === "new") {
        return [
          { label: "Dispositivos", path: internalDevicesListPath },
          {
            ...
            path: `${internalDevicesListPath}?view=new`,
          },
        ];
      }
      if (view === "edit" && id) {
        return [
          { label: "Dispositivos", path: internalDevicesListPath },
          {
            ...
            path: `${internalDevicesListPath}?id=${id}&view=edit`,
          },
        ];
      }
      if (view === "review" && id) {
        return [
          { label: "Dispositivos", path: internalDevicesListPath },
          {
            label: "Revision de Dispositivo",
            path: `${internalDevicesListPath}?id=${id}&view=review`,
          },
        ];
      }
    ...
    if (pathname === internalDevicesAssignPath && view === "new") {
      return [
        ...
        {
          label: "Asignación de Dispositivos",
          path: internalDevicesAssignPath,
        },
        {
          label: "Nueva Asignación",
          path: `${internalDevicesAssignPath}?view=new`,
        },
      ];
    }
  }
  ...
  if (
    first === "accounting" &&
    second === "requisitions" &&
    third == "requisitionsList" &&
    id &&
    !idRequisition &&
    !labelparam?.toLowerCase().startsWith("requisiciones")
  ) {
    ...
    if (!tabs.some((t) => t.label === "Detalle de Requisición")) {
      tabs = [
        ...tabs,
        { label: labelparam || "Detalle de Requisición", path: detailPath },
      ];
    }
  }
  ...
  if (
    ...
    const requisitionIdForContext =
      idRequisition ?? (view === "detail" ? id : null);
    ...
    if (requisitionsTabLabel && employeeIdForContext) {
      ...
      if (
        !tabs.some(
          (t) =>
            t.path === requisitionsPath || t.label === requisitionsTabLabel,
        )
      ) {
        tabs = [
          ...tabs,
          { label: requisitionsTabLabel, path: requisitionsPath },
        ];
      }
    }
    ...
    if (requisitionIdForContext) {
      ...
      if (requisitionsLabel)
        detailQs.set("requisitionsLabel", requisitionsLabel);
  ...
  if (
    ...
    if (view === "billablefiles") {
      ...
      if (
        !tabs.some(
          (t) => t.path === billablePath || t.label === "Archivos Facturables",
        )
      ) {
    ...
    if (view === "billablefiles") {
      ...
      if (
        !tabs.some(
          (t) =>
            t.path === billablePath ||
            t.label === "Carga de Archivos Facturables",
        )
      ) {
        tabs = [
          ...tabs,
          { label: "Carga de Archivos Facturables", path: billablePath },
        ];
      }
    }
  }
  ...
  if (
    ...
    const requisitionIdForContext =
      ...
      (!idEmployee &&
      !idRequisition &&
      (view === "detail" || view === "history" || view === "billablefiles")
        ? id
        : null);
    const requisitionsTabLabel =
      requisitionsLabel ||
    ...
    if (view === "billablefiles" && requisitionIdForContext) {
      ...
      if (requisitionsLabel)
        billableQs.set("requisitionsLabel", requisitionsLabel);
      const billablePath = `${clean}?${billableQs.toString()}`;

      if (
        !tabs.some((t) => t.path === billablePath || t.label === billableLabel)
      ) {
    ...
    if (view === "history" && requisitionIdForContext) {
      ...
      if (requisitionsLabel)
        historyQs.set("requisitionsLabel", requisitionsLabel);
      const historyPath = `${clean}?${historyQs.toString()}`;

      if (
        !tabs.some((t) => t.path === historyPath || t.label === historyTabLabel)
      ) {
    ...
    if (view === "authorizationDetail" && authorizationId) {
      ...
      if (
        !tabs.some((t) => t.path === detailPath || t.label === detailTabLabel)
      ) {
  ...
  if (first === "authorizations" && second === "authorizationslist") {
    ...
    const detailLabel =
      labelparam ||
      (authorizationKind
        ? `Solicitud de ${authorizationKind}`
  ...
  if (first === "humanresources" && second === "companies") {
    if (view === "new") {
      tabs = [
        ...
        {
          label: "Nueva empresa",
          path: "/main-page/humanresources/companies?view=new",
        },
      ];
    }
    if (view === "edit" && id) {
      tabs = [
        { label: "Empresas", path: "/main-page/humanresources/companies" },
        {
          ...
          path: `/main-page/humanresources/companies?view=edit&id=${id}`,
        },
      ];
    }
  }

  if (first === "accounting" && second === "sapkey") {
    if (view === "new") {
      tabs = [
        { label: "Claves SAP y SAT", path: "/main-page/accounting/sapkey" },
        { label: "Nueva clave", path: "/main-page/accounting/sapkey?view=new" },
      ];
    }
    ...
    if (view === "edit" && id) {
      tabs = [
        ...
        {
          label: "Editar clave",
          path: `/main-page/accounting/sapkey?view=edit&id=${id}`,
        },
      ];
    }
  }
  ...
  if (first === "humanresources" && second === "departments") {
    if (view === "new") {
      tabs = [
        {
          label: "Departamentos",
          path: "/main-page/humanresources/departments",
        },
        {
          label: "Nuevo departamento",
          path: "/main-page/humanresources/departments?view=new",
        },
      ];
    }
    if (view === "edit" && id) {
  ...
  if (
    first === "humanresources" &&
    second === "organizationchart" &&
    third === "departments"
  ) {
    if (view === "detail" && id) {
      const departmentLabel = labelparam || "Departamento";
  ...
  if (first === "proyects" && second === "proyects") {
    ...
    const baseProjectTabs: Tab[] = isNewProjectView
      ? [
          {
            label: "Proyectos",
            path: "/main-page/proyects/proyects/proyectslist",
          },
          {
            label: "Nuevo Proyecto",
            path: "/main-page/proyects/proyects/newproyect",
    ...
    if (!hasProjectContext) {
      ...
      const contextualTabs: Tab[] = [
        {
          label: "Ubicaciones",
          path: "/main-page/proyects/proyects/locations",
        },
        { label: "Dispositivos", path: "/main-page/proyects/proyects/devices" },
        {
          label: "Refacciones",
          path: "/main-page/proyects/proyects/refactions",
        },
      ].map((tab) => ({
        ...tab,
  ...
  if (first === "it" && second === "users" && third === "userspending" && id) {
    ...
    if (
      !tabs.some(
        (tab) => tab.path === activationPath || tab.label === activationLabel,
      )
    ) {
  ...
  if (
    ...
    second === "usersmanagment" &&
    third == "createemployee" &&
    idEmployee
  ) {
    tabs = tabs.map((tab) => {
      if (
        tab.path ===
          "/main-page/administration/usersmanagment/createemployee" &&
  ...
  if (first === "configuration" && second === "devices") {
    const devicesTab = {
      label: "Dispositivos",
      path: "/main-page/configuration/devices",
    };
