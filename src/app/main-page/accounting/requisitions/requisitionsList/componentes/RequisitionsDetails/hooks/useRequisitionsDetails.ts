...
const useRequisitionsDetails = () => {
    const {
        successPut, currentRequisition, gettincurrentReq, error, fetchCurrentRequisition, resetCurrentReq, resetFlags
    ...
    useEffect(() => {
        if (gettincurrentReq) { showSpinner({ message: "Obteniendo detalles de requisición" }); return; }
        hideSpinner();
        resetFlags();
        if (error) showAlert({
            type: "error",
            ...
            onSecondaryClick: () => { if (id) fetchCurrentRequisition(id) },
        });
        if (successPut && id) {
