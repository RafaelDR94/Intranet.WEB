...
vi.mock('./hooks/useVehicleRegistryList', () => ({
  __esModule: true,
  default: vi.fn(() => useVehicleRegistryListReturn),
}));
...
vi.mock('./components/RegistDetails.tsx/RegistDetails', () => {
  const React = require('react');
  return {
    __esModule: true,
...
describe('VehicleRegistryList page', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders both datatables with data', () => {
    render(<VehicleRegistryList />);
    expect(
      screen.getByTestId('datatable-Registro Vehicular en Transito')
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('datatable-Registro Vehicular')
    ).toBeInTheDocument();
  });

  it('dispatches handlers from buttons', () => {
    render(<VehicleRegistryList />);
    fireEvent.click(screen.getByTestId('refresh-Registro Vehicular en Transito'));
    expect(handleRefresh).toHaveBeenCalled();

    fireEvent.click(screen.getByTestId('button-Llegada'));
    expect(handleArrive).toHaveBeenCalledWith(
      useVehicleRegistryListReturn.inTransitRows[0].assignment
    );
    ...
    fireEvent.click(screen.getByTestId('action-Registro Vehicular'));
    expect(handleCreate).toHaveBeenCalled();
  });

  it('passes props to RegistDetails', () => {
    render(<VehicleRegistryList />);
