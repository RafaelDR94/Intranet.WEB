...
describe('EditableViaticsTable', () => {
  ...
  it('limpia valores vacíos visuales al enfocar y los restaura al salir vacíos', async () => {
    ...
    const amountInput = screen.getByLabelText('Renta de automóvil-nationalQuoted') as HTMLInputElement;
    const observationInput = screen.getByLabelText('Renta de automóvil-observations') as HTMLInputElement;
    ...
    await waitFor(() =>
      expect((screen.getByLabelText('Renta de automóvil-nationalQuoted') as HTMLInputElement).value).toBe('00'),
    );
    ...
    await waitFor(() =>
      expect((screen.getByLabelText('Renta de automóvil-observations') as HTMLInputElement).value).toBe('Escribe aquí'),
    );
  });
  ...
  it('mantiene valores capturados al enfocar', () => {
    ...
    const amountInput = screen.getByLabelText('Boleto de autobús-nationalQuoted') as HTMLInputElement;
    const observationInput = screen.getByLabelText('Boleto de autobús-observations') as HTMLInputElement;
