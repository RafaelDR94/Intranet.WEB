...
import { mapReportViewToPut } from '@/app/mappings/reports/report.mapper'
...
export const updateReport = async (
  ...
  try {
    ...
    const res: AxiosResponse = await put(Reports, mapReportViewToPut(payload))
    const raw = res.data?.data
    const updated: ReportView | null = raw ?? null

    await get().fetchAllReportsByProyect(payload?.proyect?.id,true)
