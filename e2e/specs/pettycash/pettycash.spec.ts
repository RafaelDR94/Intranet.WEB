...
test.describe('Petty Cash module', () => {
  ...
  test('muestra historial de vales con búsqueda local', async ({ page }) => {
    await page.route('**/Billings/UserDocumentsHistory/ByIdEmployee/**', async (route) => {
    ...
    await page.route('**/Billings/PettyCashVoucher/ByIdEmployee/**', async (route) => {
      if (route.request().method() === 'OPTIONS') {
        await route.fulfill({ status: 200, body: '' });
        return;
      }
    ...
    await page.route('**/Billings/PettyCashVoucher/ById/**', async (route) => {
      ...
      const voucher = pettyCashVouchers.find((item) => item.id === id) ?? pettyCashVouchers[0];
